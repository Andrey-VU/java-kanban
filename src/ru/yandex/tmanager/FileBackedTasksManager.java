...
которые были в старом, есть в новом менеджере.
    ...
    static FileBackedTasksManager loadFromFile(File file) throws IOException {   // восстанавливать
        ...
        FileBackedTasksManager loadedFromFile = new FileBackedTasksManager();
        ...
        for (String s : tmp) {
            if (s.split(",")[1].equals("Task")) {
                loadedFromFile.makeNewTask(fromString(s));
            } else if (s.split(",")[1].equals("Epic")) {
                loadedFromFile.makeNewEpic((Epic) fromString(s));
            } else {
                loadedFromFile.makeNewSubtask((Subtask) fromString(s));
            }
        }
    ...
    static List<Integer> historyFromString(String value) {
        ...
        /*
        Когда собираешь таски из файла, заведи временную мапу с тасками, ключ - id вэлью - таска.
        Потом вытащи арайлист из последней строки файла и пройдись им по мапе
        Добавляй вэлью(таску) в кастом линкедЛист (setHistory)
         */
    ...
    private void save() throws IOException {                        // сохранение изменений в файл
        ...
        try (FileWriter fileWriter = new FileWriter("/path")) {  // !!!!!!!!!!!!!!! прописать путь!!!!!!!
            ...
            // fileWriter.close();  не нужен, если try-with-resources
...
Сначала через запятую перечисляются все поля задач. Ниже находится список задач, каждая из них записана с новой 
строки. Дальше — пустая строка, которая отделяет задачи от истории просмотров. 
И заключительная строка — это идентификаторы задач из истории просмотров.
Файл из нашего примера можно прочитать так: в трекер добавлены задача, эпик и подзадача. Эпик и подзадача 
просмотрены
